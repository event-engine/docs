...
<nav class="nav-left hidden-xs hidden-sm" id="sideNav">
    <ul class="nav nav-stacked" data-spy="affix" data-offset-top="59" role="tablist">
        <?php foreach ($this->page->getHeadings() as $entry) : ?>
            ...
            // allow only headings until depth 3
            if (($entry->getLevel() - $startLevel) > 2) {
                continue;
            }

            $addSubLevel = !$isInSubLevel && ($entry->getLevel() - $startLevel) === 2;
            ...
            ?>
            <li>
                <a href="<?= $entry->getHrefAnchor() ?>" title="<?= $this->escape()->attr($entry->getTitle()) ?>">
                    <?php echo $entry->getLevel() - $startLevel === 2 ? '&nbsp;&nbsp;&nbsp;&nbsp;' : '' ?><?= $title; ?>
                </a>
            </li>
        <?php endforeach; ?>
    </ul>
